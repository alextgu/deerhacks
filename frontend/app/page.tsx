  ...
  useEffect(() => {
    ...
    function step() {
      ...
      if (!st.deleting) {
        ...
        st.c = nextC;
        if (nextC >= word.length) { st.deleting = true; timer = setTimeout(step, 2000); }
        else { timer = setTimeout(step, 72); }
      } else {
        const nextC = st.c - 1;
        setTxt(word.slice(0, nextC));
        st.c = nextC;
        if (nextC <= 0) { st.deleting = false; st.i = (st.i + 1) % texts.length; st.c = 0; timer = setTimeout(step, 380); }
        else { timer = setTimeout(step, 38); }
      }
    }
...
function TestiStrip() {
  ...
  useEffect(() => {
    startAuto();
    return () => { stopAuto(); if (idleTimer.current) clearTimeout(idleTimer.current); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const doubled = [...TESTI, ...TESTI];
  return (
    <div ref={trackRef} style={{ display:"flex", gap:"1.25rem", overflowX:"auto", paddingBottom:"1rem", cursor:"grab", scrollbarWidth:"none", userSelect:"none" }}
      onMouseDown={e => { stopAuto(); isDragging.current=true; dragStartX.current=e.pageX; dragStartSL.current=trackRef.current?.scrollLeft||0; if(trackRef.current) trackRef.current.style.cursor="grabbing"; }}
      onMouseMove={e => { if(!isDragging.current||!trackRef.current) return; e.preventDefault(); trackRef.current.scrollLeft=dragStartSL.current-(e.pageX-dragStartX.current); }}
      onMouseUp={() => { isDragging.current=false; if(trackRef.current) trackRef.current.style.cursor="grab"; scheduleRestart(); }}
      onMouseLeave={() => { if(isDragging.current){ isDragging.current=false; if(trackRef.current) trackRef.current.style.cursor="grab"; scheduleRestart(); } }}
      onTouchStart={e => { stopAuto(); dragStartX.current=e.touches[0].pageX; dragStartSL.current=trackRef.current?.scrollLeft||0; }}
      onTouchMove={e => { if(!trackRef.current) return; trackRef.current.scrollLeft=dragStartSL.current-(e.touches[0].pageX-dragStartX.current); }}
      onTouchEnd={() => scheduleRestart()}
    >
      {doubled.map((t, i) => (
        <div key={`${t.h}-${i}`} style={{ minWidth:300, flexShrink:0, background:"rgba(255,255,255,0.035)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:20, padding:"1.75rem", position:"relative", backdropFilter:"blur(8px)", transition:"border-color 0.3s, background 0.3s" }}
...
// ── Features ──────────────────────────────────────────────────────────────────
const FEATURE_ITEMS = [
  { num:"01", color:"#7c3aed", tag:"Identity",    title:"Your spider chart is your identity",       desc:"Six axes built from your actual Gemini conversations — creativity, logic, tech, writing, science, social. No quiz, no guessing.", visual:"spider" },
  { num:"02", color:"#8b2fe8", tag:"Matching",    title:"Intent-based, not swiping",                desc:"Set your goal before we show you anyone. Friend, hackathon team, or co-founder — every match is purposeful.", visual:"intent" },
  { num:"03", color:"#9333ea", tag:"Transparency",title:"See the overlap before you speak",         desc:"Both charts displayed side by side on match. Know exactly where you align before writing the first message.", visual:"overlap" },
  { num:"04", color:"#a055f5", tag:"Messaging",   title:"Instant chat, always saved",               desc:"A private panel opens the moment you match. Real-time via Supabase. No DM requests, no mutual follows.", visual:"chat" },
  { num:"05", color:"#a78bfa", tag:"Privacy",     title:"Your data never leaves the upload",        desc:"We extract interest vectors and discard the raw file immediately. Zero conversation content stored — ever.", visual:"lock" },
];
...
function FeaturesVertical({ youT, matchT }: { youT: number[]; matchT: number[] }) {
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      <div style={{ position:"absolute", left:27, top:28, bottom:28, width:1, background:"linear-gradient(to bottom, rgba(124,58,237,0.6) 0%, rgba(167,139,250,0.2) 75%, transparent 100%)", zIndex:0 }} />
      <div style={{ display:"flex", flexDirection:"column", gap:"2.5rem" }}>
        {FEATURE_ITEMS.map((item, i) => (
          <div key={item.num} style={{ display:"grid", gridTemplateColumns:"56px 1fr 320px", gap:"2rem", alignItems:"center", position:"relative", zIndex:1, opacity:inView?1:0, transform:inView?"translateX(0)":"translateX(-24px)", transition:`opacity 0.6s ${i*0.11}s, transform 0.6s ${i*0.11}s` }}>
            <div style={{ display:"flex", justifyContent:"center" }}>
              <div style={{ width:54, height:54, borderRadius:"50%", background:"var(--dark3)", border:`2px solid ${item.color}80`, display:"flex", alignItems:"center", justifyContent:"center", position:"relative", flexShrink:0, boxShadow:inView?`0 0 18px ${item.color}30`:"none", transition:"box-shadow 0.6s" }}>
                {inView && <div style={{ position:"absolute", inset:-7, borderRadius:"50%", border:`1.5px solid ${item.color}50`, animation:`pulse-ring 3s ease-out ${i*0.5}s infinite` }} />}
                <span style={{ fontSize:"0.58rem", fontWeight:800, color:item.color, letterSpacing:1 }}>{item.num}</span>
              </div>
            </div>
            <div>
              <div style={{ display:"inline-flex", alignItems:"center", background:`${item.color}14`, border:`1px solid ${item.color}30`, borderRadius:100, padding:"0.12rem 0.6rem", fontSize:"0.65rem", fontWeight:700, color:item.color, marginBottom:"0.55rem", letterSpacing:0.5 }}>{item.tag}</div>
              <h3 style={{ fontFamily:"var(--serif)", fontSize:"1.05rem", fontWeight:700, letterSpacing:"-0.3px", marginBottom:"0.4rem", color:"rgba(255,255,255,0.92)", lineHeight:1.25 }}>{item.title}</h3>
              <p style={{ fontSize:"0.81rem", color:"rgba(255,255,255,0.48)", lineHeight:1.7 }}>{item.desc}</p>
            </div>
            <div style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:16, padding:"1.25rem", display:"flex", alignItems:"center", justifyContent:"center", minHeight:130 }}>
              ...
              {item.visual === "overlap" && (
                <div style={{ position:"relative", width:110, height:110 }}>
                  ...
                  <div style={{ position:"absolute", inset:0, opacity:0.7 }}><SpiderChart values={matchT} color="#f97316" size={110} /></div>
                </div>
              )}
              {item.visual === "intent" && (
                <div style={{ display:"flex", flexDirection:"column", gap:"0.35rem", width:"100%" }}>
                  {["Friend","Hackathon (2–5)","Co-founder"].map((opt,j) => (
                    <div key={opt} style={{ background:j===0?`${item.color}14`:"rgba(255,255,255,0.03)", border:`1px solid ${j===0?item.color+"50":"rgba(255,255,255,0.07)"}`, borderRadius:8, padding:"0.45rem 0.8rem", fontSize:"0.76rem", fontWeight:600, color:j===0?"rgba(255,255,255,0.9)":"rgba(255,255,255,0.38)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      {opt}<div style={{ width:6, height:6, borderRadius:"50%", background:j===0?item.color:"rgba(255,255,255,0.12)" }} />
                    </div>
                  ))}
                </div>
              )}
              {item.visual === "chat" && (
                <div style={{ display:"flex", flexDirection:"column", gap:"0.4rem", width:"100%" }}>
                  {[{t:"them",m:"Saw our 88% match 👀"},{t:"me",m:"What are you building?"},{t:"them",m:"AI tool — same vibe"}].map((msg,j) => (
                    <div key={j} style={{ borderRadius:10, padding:"0.4rem 0.75rem", fontSize:"0.73rem", fontWeight:500, maxWidth:"78%", alignSelf:msg.t==="me"?"flex-end":"flex-start", display:"flex", background:msg.t==="me"?`linear-gradient(135deg,${item.color},#9333ea)`:"rgba(255,255,255,0.06)", border:msg.t==="me"?"none":"1px solid rgba(255,255,255,0.08)", color:"rgba(255,255,255,0.88)" }}>{msg.m}</div>
                  ))}
                </div>
              )}
              {item.visual === "lock" && (
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"0.7rem" }}>
                  <div style={{ width:48, height:48, borderRadius:"50%", background:`${item.color}14`, border:`1px solid ${item.color}30`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={item.color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                  </div>
                  <div style={{ fontSize:"0.7rem", color:"rgba(255,255,255,0.35)", textAlign:"center", lineHeight:1.5 }}>Raw file deleted on upload.<br/>Zero bytes stored.</div>
                </div>
              )}
            </div>
          </div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const onClickOutside = (e: MouseEvent) => {
      ...
      if (dropdown && wrap && !wrap.contains(e.target as Node)) dropdown.style.display = 'none';
    };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .user-avatar-fallback{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,var(--v),var(--v2));display:flex;align-items:center;justify-content:center;color:white;}
        .user-status-dot{position:absolute;bottom:0;right:0;width:10px;height:10px;border-radius:50%;background:#22c55e;border:2px solid var(--dark);}
        .user-dropdown{position:absolute;top:calc(100% + 12px);right:0;background:rgba(18,18,26,0.98);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:0.5rem;min-width:220px;backdrop-filter:blur(20px);box-shadow:0 20px 60px rgba(0,0,0,0.5),0 0 0 1px rgba(124,58,237,0.1);flex-direction:column;gap:0.15rem;z-index:300;}
        ...
        .user-dropdown-item{display:flex;align-items:center;gap:0.6rem;padding:0.6rem 0.85rem;border-radius:10px;font-size:0.82rem;font-weight:600;color:rgba(255,255,255,0.65);text-decoration:none;transition:background 0.15s,color 0.15s;cursor:pointer;}
        ...
        .testi-bg-text{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-family:var(--serif);font-size:clamp(6rem,14vw,14rem);font-weight:700;font-style:italic;color:rgba(124,58,237,0.07);white-space:nowrap;pointer-events:none;user-select:none;letter-spacing:-4px;}
        ...
        .cta-section{padding:5rem 4rem;background:var(--dark);}
        .cta-box{background:linear-gradient(135deg,#1a0a3d,#0f0a1f,#1a0a3d);border:1px solid rgba(124,58,237,0.2);border-radius:28px;padding:5rem 4rem;text-align:center;position:relative;overflow:hidden;}
        .cta-glow{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:700px;height:400px;border-radius:50%;background:radial-gradient(ellipse at center,rgba(124,58,237,0.28) 0%,rgba(100,30,200,0.14) 40%,rgba(40,10,80,0.04) 70%,transparent 100%);pointer-events:none;filter:blur(40px);}
      ...
      {/*
        ...
        Key fixes vs the old version:
          1. No more showGlow / heroHeight check — always rendered
          2. Container is position:fixed with width:0, height:0 and overflow:visible
             so the blob divs are never clipped, regardless of where on the page
             the cursor is
      */}
      <div style={{
        position: "fixed",
        ...
        <div style={{
          ...
          background: "radial-gradient(ellipse at 50% 50%, rgba(100,40,210,0.18) 0%, rgba(80,30,180,0.1) 30%, rgba(40,15,80,0.06) 58%, rgba(10,10,15,0) 80%)",
          transform: `translate(${glowPos.x - 500}px, ${glowPos.y - 340}px)`,
          filter: "blur(55px)",
        ...
        <div style={{
          ...
          background: "radial-gradient(ellipse at 48% 52%, rgba(130,55,240,0.32) 0%, rgba(110,40,220,0.18) 35%, rgba(60,20,120,0.07) 62%, rgba(10,10,15,0) 78%)",
          transform: `translate(${glowPos.x - 280}px, ${glowPos.y - 200}px)`,
          filter: "blur(36px)",
          willChange: "transform",
        }} />
        {/* Tight bright core */}
        <div style={{
          ...
          width: 260,
          height: 190,
          borderRadius: "68% 32% 52% 48% / 42% 58% 42% 58%",
          background: "radial-gradient(ellipse at 46% 48%, rgba(155,90,255,0.42) 0%, rgba(130,60,240,0.22) 40%, rgba(70,25,140,0.06) 68%, rgba(10,10,15,0) 80%)",
          transform: `translate(${glowPos.x - 130}px, ${glowPos.y - 95}px) rotate(18deg)`,
          filter: "blur(20px)",
          willChange: "transform",
        }} />
      </div>
      ...
      <section ref={heroRef} className="hero">
        ...
        <div className="hero-left">
          <h1 className="hero-h1">Connect Through <br /><em>Shared</em><br />Thinking.</h1>
      ...
      <section id="how" className="how-section">
        ...
        <RevealDiv><p className="section-sub">Like depth-first search — we go deep before we go wide.<br />Five steps, one algorithm.</p></RevealDiv>
        <RevealDiv><HowTree /></RevealDiv>
      </section>
      ...
      <section id="features" className="features-section">
        ...
        <RevealDiv style={{ marginTop:"3rem" }}><FeaturesVertical youT={youT} matchT={matchT} /></RevealDiv>
      </section>
        ...
        <div className="testi-inner">
          ...
          <RevealDiv style={{ marginTop:"2.5rem" }}>
            ...
            <div className="drag-hint">
              <div className="drag-hint-line" />← drag or let it flow →<div className="drag-hint-line" />
            </div>
          </RevealDiv>
        </div>
      </section>
        ...
        <div className="footer-top">
          ...
          <div>
            ...
            <div className="footer-links">
              ...
              <a href="#feedback">Testimonials</a><a href="#">Changelog</a>
            </div>
          </div>
          <div>
            ...
            <div className="footer-links">
              ...
              <a href="#">GitHub</a><a href="#">DeerHacks 2025</a>
            </div>
          </div>
          <div>
            ...
            <div className="footer-links">
              ...
              <a href="#">Cookie Policy</a><a href="#">Security</a>
            </div>
          </div>
        </div>
        <div className="footer-bottom">
          ...
          <div className="footer-socials">
            <div className="footer-social-link">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"/></svg>
            </div>
            <div className="footer-social-link">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/></svg>
            </div>
            <div className="footer-social-link">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>
            </div>
          </div>
        ...
        {HOW_STEPS.map((step, i) => (
          <div key={step.num} style={{ display:"flex", alignItems:"flex-start", flex:1 }}>
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:0 }}>
              <div style={{ fontSize:"0.6rem", fontWeight:800, color:step.color, letterSpacing:2, marginBottom:10, opacity:inView?1:0, transition:`opacity 0.4s ${i*0.12}s` }}>{step.num}</div>
              <div style={{ width:72, height:72, borderRadius:"50%", background:"var(--dark3)", border:`2px solid ${step.color}80`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.5rem", position:"relative", opacity:inView?1:0, transform:inView?"scale(1)":"scale(0.5)", transition:`opacity 0.5s ${i*0.12}s, transform 0.5s ${i*0.12}s`, flexShrink:0, boxShadow:inView?`0 0 20px ${step.color}35`:"none" }}>
                {inView && <div style={{ position:"absolute", inset:-8, borderRadius:"50%", border:`2px solid ${step.color}55`, animation:`pulse-ring 2.5s ease-out ${i*0.5}s infinite` }} />}
                {inView && <div style={{ position:"absolute", inset:-16, borderRadius:"50%", border:`1.5px solid ${step.color}30`, animation:`pulse-ring 2.5s ease-out ${i*0.5+0.6}s infinite` }} />}
                <span style={{ color:step.color, fontFamily:"var(--serif)", fontSize:"1.2rem" }}>{step.icon}</span>
              </div>
              <div style={{ width:1, height:20, background:`linear-gradient(to bottom,${step.color}70,transparent)`, opacity:inView?1:0, transition:`opacity 0.4s ${i*0.12+0.3}s` }} />
              <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, padding:"0.85rem 0.75rem", textAlign:"center", width:"100%", opacity:inView?1:0, transform:inView?"translateY(0)":"translateY(10px)", transition:`opacity 0.5s ${i*0.12+0.2}s, transform 0.5s ${i*0.12+0.2}s` }}>
            ...
            {i < HOW_STEPS.length - 1 && (
              <div style={{ display:"flex", alignItems:"center", marginTop:46, width:40, flexShrink:0, opacity:inView?1:0, transition:`opacity 0.4s ${i*0.12+0.4}s` }}>
